...
![test image size](/images/BN/grads.png#thumbnail){:height="70%" width="80%"}
...
![test image size](/images/BN/bn.png#thumbnail){:height="60%" width="60%"}
...
$$\;\;\;\;=-\frac{\gamma_i}{\mu_i}\sum_{j=1}^{m}\frac{\partial L}{\partial a_i^{(j)}}+\left(\frac{1}{\sigma_i^3}\right)\left(\sum_{j=1}^{m}\frac{\partial L}{\partial a_i^{(j)}}\gamma_i(x_i^{(j)} - \mu_i)\right) \left(\frac{\sum_{j=1}^{m}(x_i^{(j)} - \mu_i)}{m}\right)$$
