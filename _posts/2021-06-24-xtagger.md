...
Second model of x-tagger is LSTM (both unidirectional and bidirectional). For bidirectional case, given a sequence of $$N$$ words $$(t_1, t_2, ..., t_N)$$, a forward part-of-speech tagger computes the probability of the sequence by modeling the probability of tag $t_k$ given the history $$(t_1, t_2, ... t_{k-1})$$

$$p(t_1, t_2, ..., t_N) = \prod_{k=1}^N p(t_k \mid w_1, w_2, ..., w_{k-1})$$

The backward part-of-speech tagger is similar to forward part-of-speech tagger:

$$p(t_1, t_2, ..., t_N) = \prod_{k=1}^N p(t_k \mid w_{k+1}, w_{k+2}, ..., w_{N})$$
